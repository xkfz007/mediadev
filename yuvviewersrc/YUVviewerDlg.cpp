	...
	while(g_nCurrentFrame >= g_nStartFrame && g_nCurrentFrame <= g_nEndFrame && !bEof)
	{
		...
		for(i=0; i<pWin->m_iCount; i++)
		{
			...
			if(picsize != pWin->m_pFile[i]->Read(pWin->m_pWnd[i]->Y,picsize))
			{
                ...
                /// pWin->ResetClips();
            	pWin->m_buttonPausePlay.SetWindowText("Play");
                pWin->m_bEOF    = TRUE;
                pWin->m_bPlay   = FALSE;
...
void CYUVviewerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SIZE_CIF, m_rdoSizeCIF);
...
BEGIN_MESSAGE_MAP(CYUVviewerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SIZE_SELECT, OnSizeSelect)
	ON_CBN_SELCHANGE(IDC_CMB_SIZE, OnSelchangeCmbSize)
	//}}AFX_MSG_MAP
	ON_EN_CHANGE(IDC_SIZE_WIDTH, &CYUVviewerDlg::OnEnChangeSizeWidth)
END_MESSAGE_MAP()
	/// ON_BN_CLICKED(IDC_OPENFILE, OnOpenfile)
...
void CYUVviewerDlg::OnSelchangeCmbSize() 
{
	// TODO: Add your control notification handler code here
    int cntWidth[]  = {352, 320, 160};
    int cntHeight[] = {240, 240, 120};
	int nIndex = m_cmbSize.GetCurSel();

	m_nWidth    = cntWidth[nIndex];
	m_nHeight   = cntHeight[nIndex];
...
void CYUVviewerDlg::OnSizeSelect() 
{
	// TODO: Add your control notification handler code here
    Enable(IDC_CMB_SIZE);
...
void CYUVviewerDlg::OnEnChangeSizeWidth()
{
