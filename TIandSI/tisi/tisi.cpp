...
bool SortFunc( double v1, double v2)
{  
    return v1 > v2;
}  
...
int TIandSI(const char* strPath,int width,int height,int pixfmt)
{
    ...
    double *NewSIBuffer;
    double *NewTIBuffer;
    double flSI_A, flSI_S, flTI_A, flTI_S;
    ...
    NewSIBuffer = new double [nYSize];
    NewTIBuffer = new double [nYSize];

    fYUV=fopen(strPath,"r");
    if(fYUV==NULL)
    {
...
int main(int argc, _TCHAR* argv[])
{
    ...
    while ((opt =getopt(argc, argv,"i:w:h:f:H")) != -1)
    {
        switch (opt)
        {
        case 'H':
